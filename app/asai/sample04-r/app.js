...
var ComboBoxDirective = (function () {
    function ComboBoxDirective() {
        ...
        this.restrict = 'EA';
        this.transclude = true; // notification タグ内の値をng-transclude内に要素がappendされる
        this.replace = true; // trueの場合でもnotification タグの属性も引き継がれる
        // ` を付けると + を使わなくてよくなる、angular expression が使えるようになるよ
        this.template = '<div class="comboBox">' + '<input type="text" ng-model="selectedItem">' + '<ul ng-show="isFocus">' + '<li ng-repeat="item in allItems" ng-click="click($event, item)">' + '{{item}}' + '</li>' + '</ul>' + '</div>';
...
app.directive('refComboBox', function () { return new ComboBoxDirective(); });
/**
 * 夕日本 P.275~
 */
